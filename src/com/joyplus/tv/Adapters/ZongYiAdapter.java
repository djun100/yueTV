...
//		viewItemHodler.haibaoIv.setBackgroundResource(R.drawable.post_normal);	
		...
		if(viewItemHodler.haibaoIv.getTag()==null || 	!viewItemHodler.haibaoIv.getTag().equals(movieList.get(position).getMoviePicUrl())){
...
//		loader.loadBitmap(movieList.get(position).getMoviePicUrl(), position);
		...
		aq.id(viewItemHodler.haibaoIv).image(movieList.get(position).getMoviePicUrl(), 	false, true,0, R.drawable.post_normal);
		return convertView;
	}
